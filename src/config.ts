...
//
// [시트 구성 — 1행은 헤더]
//  name | 성 | 한글이름 | age | gender | 신장체중 | 종족 | 소속및직업 | tags | image | 성격 | 특징 | 외관 | 기타 | ...
//
//  ─ 고정 의미의 열(=탭으로 안 만들어짐): name, 성, age(나이), gender(성별), tags(태그), image(이미지)
//  ─ name = 영문이름(개요 큰 제목 + 상세 헤더에 표시). 기본정보의 "영문이름"이 이 값.
//  ─ "기본정보" 탭에 표시되는 열:
//        한글이름 / 영문이름(=name) / 성 / 나이 / 성별 / 신장체중 / 종족 / 소속및직업
